...
def prepare_DT_df_steel_faults():
    df = df_fitting_and_evaluation_steel_faults()
    ...
    df['Steel_Faults_encoded'] = label_encoder.fit_transform(df['Steel_Faults'])
    #
    ...
    df = df.drop(columns=['Steel_Faults'])
...
def Probabilistic_Decision_Tree_Steel_Faults(depth, persist=False):
    df = prepare_DT_df_steel_faults()
    #print(df.head())
    X = df.iloc[:, 0:27]
    y = df.iloc[:, 27]
    #print('x: ',X)
    ...
    if persist:
        save_model_artifact(dtc, STEEL_FAULTS_PROBABILISTIC_MODEL_PATH)
