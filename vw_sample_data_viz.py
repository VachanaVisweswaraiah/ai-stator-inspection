...
def load_model_vw_sample_pdt():
    ...
    vw_sample_probabilistic_decision_tree_model = load_vw_sample_probabilistic_model()
    return vw_sample_probabilistic_decision_tree_model

def predict_input_pdt(df_input_val):
    probabilistic_decision_tree_model = load_model_vw_sample_pdt()
...
def vw_sample_probabilistic_decision_tree_viz(depth,selected_to_drop):
    ...
    tab0, tab1, tab2, tab4, tab5 = st.tabs(
        ["User Prediction", "Confusion-Matrix", "Evaluation-Metrics", "VW Sample PDT Visualization", "Custom Insights"])
        ...
        if option == "Manual Input":
            ...
            if uploaded_file is not None:
                ...
                if st.button("Make Prediction"):
                    ...
                    color_mapping = {
                        'NIO': 'background-color: rgba(255, 99, 132, 0.3)',        # Red
                        'IO': 'background-color: rgba(144, 238, 144, 0.3)'    # light green
                    }
    ...
    with tab1:
        ...
        confusion_matrix_df = pd.DataFrame(confusion_matrix_test, index=['IO','NIO'],
                                           columns=['IO','NIO'])
    ...
    with tab4:
        ...
        def visualize_vw_sample_probabilistic_decision_tree(dtc, feature_names):
            ...
            path_filter = st.selectbox("🔍 Filter & Show Paths", ['All','IO','NIO'])
            class_map = {"IO": 0, "NIO": 1}
            ...
            def traverse(node_idx, depth=0, pos_x=100, pos_y=100):
                ...
                if is_leaf:
                    ...
                    color_map = {
                            1: "#FFCCCB",  # light red for NIO
                            0: "#90EE90",  # light green for IO
                        }
    ...
    with tab5:
        ...
        if 'tree_screenshot_vw_sample' not in st.session_state:
            st.session_state['tree_screenshot_vw_sample'] = None
        
        if 'tree_screenshot_dt_vw_sample' not in st.session_state:
            st.session_state['tree_screenshot_dt_vw_sample'] = None
        ...
        if 'show_tree_canvas_pdt_vw_sample' not in st.session_state:
            st.session_state['show_tree_canvas_pdt_vw_sample'] = False

        # Toggle button for tree canvas
        if not st.session_state['show_tree_canvas_pdt_vw_sample']:
            if st.button("Create your own Tree"):
                st.session_state['show_tree_canvas_pdt_vw_sample'] = True
                st.rerun()
        else:
        ...
        # Tree Canvas Section
        if st.session_state['show_tree_canvas_pdt_vw_sample']:
            # Initialize canvas state if not present
            if 'canvas_state_dt_vw_sample' not in st.session_state:
                st.session_state.canvas_state_dt_vw_sample = StreamlitFlowState([], [])
            ...
            with tree_container:
                ...
                # Draw flow component
                st.session_state.canvas_state_dt_vw_sample = streamlit_flow(
                    key='fully_interactive_flow',
                    state=st.session_state.canvas_state_dt_vw_sample,
                    fit_view=True,
                    show_controls=True,
                ...
                if st.checkbox("Debug Node Structure", False):
                    ...
                    node_debug_list = []
                    for node in st.session_state.canvas_state_dt_vw_sample.nodes:
            ...
            col1.metric("Nodes", len(st.session_state.canvas_state_dt_vw_sample.nodes))
            col2.metric("Edges", len(st.session_state.canvas_state_dt_vw_sample.edges))

            st.markdown("---")

                                # Create a PNG representation of the tree (server-side)
            if len(st.session_state.canvas_state_dt_vw_sample.nodes) > 0:
                try:
                    ...
                    st.session_state['node_debug_info_vw_sample'] = node_debug_info
                    
                    # Add edges
                    for edge in st.session_state.canvas_state_dt_vw_sample.edges:
                        edge_label = None
                    ...
                    st.session_state['tree_screenshot_dt_vw_sample'] = buf.getvalue()
            ...
            tree_data = {
                "nodes": [node.__dict__ for node in st.session_state.canvas_state_dt_vw_sample.nodes],
                "edges": [edge.__dict__ for edge in st.session_state.canvas_state_dt_vw_sample.edges]
            }
            ...
            if st.session_state['tree_screenshot_dt_vw_sample'] is not None:
                col2.download_button(
                    label="Download Tree Image",
                    data=st.session_state['tree_screenshot_dt_vw_sample'],
                    file_name=f"tree_visualization_{int(time.time())}.png",
                    mime="image/png"
                )
            
        # Initialize hypothesis state
        if "submitted_hypotheses_vw_sample" not in st.session_state:
            st.session_state["submitted_hypotheses_vw_sample"] = []
        
        # Initialize a reset flag
        if "reset_form_dt_vw_sample" not in st.session_state:
            st.session_state["reset_form_dt_vw_sample"] = False
            
        # Initialize default values for form fields
        if "reset_form_dt_vw_sample" in st.session_state and st.session_state["reset_form_dt_vw_sample"]:
            form_defaults = {
                "failure_desc": "",
                "imp_params": [],
                "failure_name": "",
            ...
            form_defaults = {
                "failure_desc": st.session_state.get("failure_desc_dt_vw_sample", ""),
                "imp_params": st.session_state.get("imp_params_dt_vw_sample", ""),
                "failure_name": st.session_state.get("failure_name_dt_vw_sample", ""),
                "hypo_prob": st.session_state.get("hypo_prob_dt_vw_sample", "Medium"),
                "fail_imp": st.session_state.get("fail_imp_dt_vw_sample", "Medium")
            }
                ...
                with st.form(key="domain_hypothesis_form_dt"):
                    ...
                    if submitted:
                        ...
                        if not failure_description or not failure_name:
                            ...
                            st.session_state["submitted_hypotheses_vw_sample"].append(entry)
                    ...
                    if clear_form:
                        ...
                        st.session_state["reset_form_dt_vw_sample"] = True
                        st.rerun()

        # Display and export submitted hypotheses
        if st.session_state["submitted_hypotheses_vw_sample"]:
            with st.expander("View All Submitted Hypotheses", expanded=False):
                df_hypo = pd.DataFrame(st.session_state["submitted_hypotheses_vw_sample"])
                st.dataframe(df_hypo)
                
            csv_data = pd.DataFrame(st.session_state["submitted_hypotheses_vw_sample"]).to_csv(index=False).encode("utf-8")
            st.download_button(
